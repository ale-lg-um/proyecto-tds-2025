...
public class ServicioAlertas {
	...
	public static ServicioAlertas getInstancia() {
		if (instancia == null)
			instancia = new ServicioAlertas();
		return instancia;
	}

	public void guardarNuevaAlerta(Cuenta cuenta, String tipo, String limiteTxt, Categoria cat)
	...
	public String obtenerTextoDescriptivo(Alerta item) {
		...
		return String.format("%s %s -> %.2f€ (%s)", item.getTipo(), rango, item.getLimite(), nombreCat);
	}
	...
	public Alerta comprobarAlertas(Cuenta cuenta, Gasto nuevoGasto) {
		...
		for (Alerta alerta : cuenta.getAlertas()) {
			...
			if (alerta.esMismaCategoria(nuevoGasto)) {
				if (verificarLimite(alerta, cuenta, nuevoGasto))
					return alerta;
			}
		}
		for (Alerta alerta : cuenta.getAlertas()) {
