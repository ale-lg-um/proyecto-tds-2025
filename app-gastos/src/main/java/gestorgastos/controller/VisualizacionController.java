...
public class VisualizacionController {
	...
	@FXML
	private ListView<Month> listMeses;
	@FXML
	...
	/*
	 * public void setCuenta(Cuenta cuenta) { this.cuentaActual = cuenta; // Cargar
	 * las categorías de esta cuenta en el filtro
	 * listCategorias.getItems().setAll(cuenta.getCategorias());
	 * 
	 * // Mostrar todo por defecto al entrar aplicarFiltros(); }
	 */
	...
	/*
	 * @FXML private void aplicarFiltros() { //Cuenta cuentaActual =
	 * SesionService.getInstancia().getCuentaActiva(); if (cuentaActual == null)
	 * return;
	 * 
	 * List<Gasto> todos = cuentaService.obtenerGastos(cuentaActual);
	 * 
	 * List<Gasto> filtrados = todos.stream() // Filtrar por rango de fechas
	 * .filter(g -> { if (dateDesde.getValue() != null &&
	 * gastosServices.obtenerFecha(g).isBefore(dateDesde.getValue())) return false;
	 * if (dateHasta.getValue() != null &&
	 * gastosServices.obtenerFecha(g).isAfter(dateHasta.getValue())) return false;
	 * return true; }) // Filtrar por meses seleccionados .filter(g -> { List<Month>
	 * mesesSel = listMeses.getSelectionModel().getSelectedItems(); if
	 * (mesesSel.isEmpty()) return true; // Si no hay nada seleccionado, mostrar
	 * todo return mesesSel.contains(gastosServices.obtenerFecha(g).getMonth()); })
	 * // Filtrar por categorías seleccionadas .filter(g -> { List<Categoria>
	 * catsSel = listCategorias.getSelectionModel().getSelectedItems(); if
	 * (catsSel.isEmpty()) return true; // Comparamos por nombre para asegurar
	 * return catsSel.stream().anyMatch(c ->
	 * categoriasService.getNombre(c).equals(categoriasService.getNombre(
	 * gastosServices.obtenerCategoria(g)))); }) .collect(Collectors.toList());
	 * 
	 * // Actualizar gráficos y calendario actualizarGraficos(filtrados);
	...
	private void aplicarFiltros() {
		...
		// Obtenemos copias de las listas seleccionadas
		List<Month> mesesSeleccionados = listMeses.getSelectionModel().getSelectedItems();
	 ...
	 * 
	 * pieChart.setAnimated(false); barChart.setAnimated(false);
	 * 
	 ...
	 * cuentaService.obtenerCategorias(cuentaActual).stream()
	 * .collect(Collectors.toMap(categoriasService::getNombre,
	 * categoriasService::obtenerColorHex, (a, b) -> a));
	 * 
	 * pieChart.getData().clear(); porCategoria.forEach((catName, total) -> {
	 ...
	 * 
	 * porCategoria.forEach((catName, total) -> { series.getData().add(new
	 * XYChart.Data<>(catName, total)); });
	 * 
	 * barChart.getData().add(series);
	 * 
	 * for (XYChart.Data<String, Number> data : series.getData()) {
	 * javafx.scene.Node barra = data.getNode(); if (barra != null) { String catName
	 * = data.getXValue(); String color = mapaColores.getOrDefault(catName,
	 * "#808080"); barra.setStyle("-fx-bar-fill: " + color + ";"); } }
	 * 
	 * Platform.runLater(() -> { Node legend = pieChart.lookup(".chart-legend");
	 * 
	 * if (legend != null && legend instanceof Parent) { for (Node item : ((Parent)
	 * legend).getChildrenUnmodifiable()) { if (item instanceof Label) { Label label
	 * = (Label) item; String catName = label.getText();
	 * 
	 * if (mapaColores.containsKey(catName)) { String color =
	 * mapaColores.get(catName);
	 * 
	...
	private void actualizarGraficos(List<Gasto> gastos) {
		...
		Map<String, Double> porCategoria = cuentaService.agruparGastosPorCategoria(gastos);
		Map<String, String> mapaColores = cuentaService.obtenerMapaColoresCategorias(cuentaActual);
	...
	private void volver() {
		try {
			...
			FXMLLoader loader = new FXMLLoader(
					getClass().getResource("/gestorgastos/app_gastos/DetalleCuentaView.fxml"));
